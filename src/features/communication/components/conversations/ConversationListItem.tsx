...
function localizedValue(
  ...
  const fallback = locale === "ar" ? conversation.titleEn : conversation.titleAr;
...
export default function ConversationListItem({
  ...
  return (
    <article className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm transition-colors hover:border-sky-200">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex shrink-0 items-start gap-1">
          {conversation.isPinned ? (
            <Pin className="mt-2 h-4 w-4 text-sky-600" aria-hidden="true" />
          ) : null}
          {typeof conversation.unreadCount === "number" &&
          conversation.unreadCount > 0 ? (
            <span className="mt-1 inline-flex min-w-6 items-center justify-center rounded-full bg-sky-600 px-2 py-1 text-xs font-semibold text-white">
