...
import type { TeachersExportFormat } from "@/features/teachers/shared/utils/teachersExport";

interface TeachersGlobalExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onExport: (format: TeachersExportFormat) => Promise<void> | void;
  title?: string;
  subtitle?: string;
  confirmLabel?: string;
  datasetCount?: number;
...
export default function TeachersGlobalExportModal({
  isOpen,
  onClose,
  onExport,
  title,
  ...
  datasetCount,
  emptyStateMessage,
}: TeachersGlobalExportModalProps) {
  const t = useTranslations("teachers.export");
  const [format, setFormat] = useState<TeachersExportFormat>("csv");
