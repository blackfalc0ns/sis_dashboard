...
import type { ReinforcementExportFormat } from "@/features/reinforcement/shared/utils/reinforcementExport";

interface ReinforcementExportDatasetOption {
  value: string;
  label: string;
  description?: string;
}

interface ReinforcementGlobalExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onExport: (format: ReinforcementExportFormat) => Promise<void> | void;
  title?: string;
  subtitle?: string;
  confirmLabel?: string;
  datasetCount?: number;
  emptyStateMessage?: string;
  datasetOptions?: ReinforcementExportDatasetOption[];
  selectedDataset?: string;
  onDatasetChange?: (value: string) => void;
}

export default function ReinforcementGlobalExportModal({
  isOpen,
  onClose,
  onExport,
  title,
  ...
  selectedDataset,
  onDatasetChange,
}: ReinforcementGlobalExportModalProps) {
  const t = useTranslations("reinforcement.export");
  const [format, setFormat] = useState<ReinforcementExportFormat>("csv");
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      console.error("Reinforcement export error:", error);
      alert(
