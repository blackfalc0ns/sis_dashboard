...
export interface AdmissionsUrlQueryStateConfig<
...
export interface AdmissionsUrlQueryStateResult<
  T extends Record<string, string>,
> {
  values: T;
...
export function useAdmissionsUrlQueryState<T extends Record<string, string>>(
  config: AdmissionsUrlQueryStateConfig<T>,
): AdmissionsUrlQueryStateResult<T> {
  const {
    defaults,
    debouncedKeys = [],
  ...
  const setValues = useCallback(
    (
      ...
      setValuesState((current) => {
        ...
        Object.entries(updates).forEach(([key, rawValue]) => {
          const typedKey = key as keyof T;
          nextValues[typedKey] = (rawValue ??
            defaultValues[typedKey]) as T[keyof T];
        });
        return nextValues;
      });
  ...
  const setValue = useCallback(
    <K extends keyof T>(
      ...
      setValues(
        { [key]: value } as Partial<Record<keyof T, string | null>>,
        modeOverride,
      );
    },
    [setValues],
  ...
  useEffect(() => {
    ...
    if (!normalizedUpdates || Object.keys(normalizedUpdates).length === 0) {
      return;
    }
