...
export default function EnrollmentList() {
  ...
  const load = useCallback(async () => {
    ...
    catch { setError("Unable to load enrollments."); } finally { setLoading(false); }
  }, [canView, status]);
  ...
  useEffect(() => { if (!yearId || !termId) return; void fetchStructureTree(yearId, termId).then((tree) => { const name = (item: { name?: string; nameAr?: string; nameEn?: string }) => locale === "ar" ? item.nameAr || item.name || "—" : item.nameEn || item.name || "—"; setGrades(tree.grades.map((item) => ({ id: item.id, name: name(item) }))); setSections(tree.sections.map((item) => ({ id: item.id, name: name(item), parentId: item.gradeId }))); setClassrooms(tree.classrooms.map((item) => ({ id: item.id, name: name(item), parentId: item.sectionId }))); }).catch(() => { setGrades([]); setSections([]); setClassrooms([]); }); }, [locale, termId, yearId]);
  ...
  const columns = [
    { key: "studentName", label: t("student_name") }, { key: "status", label: "Status" }, { key: "academicYear", label: t("academic_year") }, { key: "grade", label: t("grade") }, { key: "section", label: t("section") }, { key: "classroom", label: t("classroom") }, { key: "enrollmentDate", label: t("enrolled_date"), render: (value: unknown) => new Date(String(value)).toLocaleDateString(locale) },
  ];
  if (!canView) return <AdmissionsAccessDenied />;
  return <div className="space-y-6">
    <div className="grid gap-4 md:grid-cols-3"><KPICardV2 title={t("total_enrolled")} value={records.length} icon={Users} iconColor="#3b82f6" iconBgColor="#dbeafe" /><KPICardV2 title="Active" value={records.filter((item) => item.status === "active").length} icon={CheckCircle} iconColor="#10b981" iconBgColor="#d1fae5" /><KPICardV2 title={t("this_week")} value={thisWeek} icon={Calendar} iconColor="#8b5cf6" iconBgColor="#ede9fe" /></div>
    {isReadOnly && <AdmissionsReadOnlyBanner />}
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-xl font-bold">{t("title")}</h2><p className="text-sm text-gray-500">{t("subtitle")}</p></div>{canManage && <Button type="button" leftIcon={<Plus className="h-4 w-4" />} onClick={() => { setEditing(null); setPlacementOpen(true); }}>New enrollment</Button>}</div>
    <FilterPanel searchSlot={<div className="max-w-md flex-1"><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t("search_placeholder")} leftIcon={<Search className="h-4 w-4" />} /></div>} filtersSlot={<Select value={status} onChange={(value) => setStatus(value as EnrollmentStatusDto | "all")} options={[{ value: "all", label: "All statuses" }, { value: "active", label: "Active" }, { value: "completed", label: "Completed" }, { value: "withdrawn", label: "Withdrawn" }]} />} showFilters onToggleFilters={() => undefined} clearAction={null} hasActiveFilters={Boolean(search || status !== "all")} />
    {loading ? <div className="rounded-xl bg-white p-10"><PartialLoader /></div> : error ? <div className="rounded-xl bg-red-50 p-5 text-red-700">{error}<Button type="button" variant="ghost" size="sm" className="ml-3 text-red-700" onClick={() => void load()}>Retry</Button></div> : visible.length === 0 ? <div className="rounded-xl bg-white"><EmptyState message="No enrollments found." /></div> : <DataTable columns={columns} data={visible as (EnrollmentRecord & { [key: string]: unknown })[]} onRowClick={setSelected} searchQuery={search} />}
