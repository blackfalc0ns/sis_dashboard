...
export default function EnrollmentPlacementDialog({ open, enrollment, students, academicYears, grades, sections, classrooms, termId, onClose, onSuccess }: Props) {
  ...
  const submit = async () => {
    ...
    catch { setErrors([t("dialogs.placement.save_error")]); } finally { setSaving(false); }
  };
  return <Modal isOpen onClose={onClose} title={enrollment ? t("dialogs.placement.edit_title") : t("dialogs.placement.new_title")} size="lg" footer={<><Button variant="outline" onClick={onClose}>{t("cancel")}</Button><Button onClick={() => void submit()} loading={saving} disabled={!studentId || !academicYearId || !classroomId}>{t("actions.save")}</Button></>}><div className="grid grid-cols-1 gap-4 md:grid-cols-2">
    <Select label={t("dialogs.placement.student")} value={studentId} disabled={Boolean(enrollment)} onChange={setStudentId} searchable options={students.map((student) => ({ value: student.id, label: studentDisplayName(student, locale) }))} />
    <Select label={t("dialogs.placement.academic_year")} value={academicYearId} onChange={setAcademicYearId} options={academicYears.map((year) => ({ value: year.id, label: locale === "ar" ? year.nameAr || year.name : year.nameEn || year.name }))} />
    <Select label={t("dialogs.placement.grade")} value={gradeId} onChange={(value) => { setGradeId(value); setSectionId(""); setClassroomId(""); }} options={grades.map((option) => ({ value: option.id, label: option.name }))} />
    <Select label={t("dialogs.placement.section")} value={sectionId} onChange={(value) => { setSectionId(value); setClassroomId(""); }} options={shownSections.map((option) => ({ value: option.id, label: option.name }))} />
    <Select label={t("dialogs.placement.classroom")} value={classroomId} onChange={setClassroomId} options={shownClassrooms.map((option) => ({ value: option.id, label: option.name }))} />
    <Input label={t("dialogs.placement.enrollment_date")} type="date" value={enrollmentDate} onChange={(e) => setEnrollmentDate(e.target.value)} />
