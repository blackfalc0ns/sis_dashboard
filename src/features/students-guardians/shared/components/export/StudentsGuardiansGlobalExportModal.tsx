...
import type { StudentsGuardiansExportFormat } from "@/features/students-guardians/shared/utils/studentsGuardiansExport";

interface StudentsGuardiansGlobalExportModalProps {
  isOpen: boolean;
  onClose: () => void;
  onExport: (format: StudentsGuardiansExportFormat) => Promise<void> | void;
  title?: string;
  subtitle?: string;
  confirmLabel?: string;
  datasetCount?: number;
...
export default function StudentsGuardiansGlobalExportModal({
  isOpen,
  onClose,
  onExport,
  title,
  ...
  datasetCount,
  emptyStateMessage,
}: StudentsGuardiansGlobalExportModalProps) {
  const t = useTranslations("admissions.export");
  const [format, setFormat] = useState<StudentsGuardiansExportFormat>("csv");
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      console.error("Students & Guardians export error:", error);
      alert(
