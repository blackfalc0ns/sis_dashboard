...
export default function HeroJourneyTab({ student, academicYearId, termId }: { student: Student; academicYearId?: string | null; termId?: string | null }) {
  ...
  const grantXp = async () => {
    if (!activeProgressId || !Number.isFinite(Number(amount)) || Number(amount) <= 0) return;
  ...
  return <div className="space-y-4">
    ...
    <Modal isOpen={isXpModalOpen} onClose={() => setIsXpModalOpen(false)} title={t("grantHeroXp")} size="sm" footer={<><Button type="button" variant="secondary" onClick={() => setIsXpModalOpen(false)}>{t("cancel")}</Button><Button type="button" onClick={() => void grantXp()} disabled={!activeProgressId || Number(amount) <= 0}>{t("grantXp")}</Button></>}>
      <label htmlFor="profile-hero-xp" className="block text-sm font-medium text-gray-700">{t("xpAmount")}</label>
      <input id="profile-hero-xp" aria-label="XP amount" type="number" min="1" value={amount} onChange={(event) => setAmount(event.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2" />
    </Modal>
  </div>;
}
