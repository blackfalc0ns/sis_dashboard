...
interface StudentAccountLinkModalProps {
  isOpen: boolean;
...
function getStudentContactEmail(student: Student | null) {
  return student?.contact?.student_email || "";
}
...
export default function StudentAccountLinkModal({
  isOpen,
  student,
  onClose,
  onLinked,
}: StudentAccountLinkModalProps) {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (mode === "create" && !contactEmail.trim()) {
      setError(t("validation.contact_email_required"));
      return;
    }
    ...
    setLinkedResponse(null);
    try {
      const response = await linkStudentAccount(student.id, {
        mode,
  ...
  return (
    <Modal
      ...
      description={student ? t("for_record", { name: student.full_name_en }) : ""}
    >
