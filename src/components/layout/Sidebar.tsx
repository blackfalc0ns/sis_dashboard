...
export default function Sidebar({
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed z-50 h-screen bg-white flex flex-col transition-all duration-300 ease-in-out ${isRTL ? "right-0 border-l" : "left-0 border-r"} border-gray-200 ${isOpen ? "translate-x-0" : isRTL ? "translate-x-full lg:translate-x-0" : "-translate-x-full lg:translate-x-0"} ${isOpen ? "w-[260px] p-2" : "w-[260px] lg:w-20 lg:px-3"}`}
      >
        <button
          onClick={onToggle}
                        ...
                        return (
                          <div key={child.key}>
                            ...
                            {hasGrandchildren && isChildExpanded && (
                              <div
                                className={`relative mt-1 space-y-1 ${isArabic ? "mr-4" : "ml-4"}`}
                              >
