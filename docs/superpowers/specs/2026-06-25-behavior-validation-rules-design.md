...
- **Category Points Validation:**
  - `validateCategoryPoints(type: BehaviorType, points: number): boolean`:
    - Points must be an integer (no decimals).
...
- **Record Term Date Validation:**
  - `validateRecordTermDate(occurredAt: string | Date, termRange?: { startDate: string; endDate: string }): boolean`: Checks if `occurredAt` date falls inside the selected term's date range (if term data is available).
...
- **Review Points Override Validation:**
  - `validatePointsOverride(type: BehaviorType, points: number): boolean`: Checks that the overridden points are an integer and sign-compatible with the record type.
...
- **On Edit Constraint:** If category usage counts are known and the category is in use:
...
- **Validation on Save:**
  ...
  - If a category is selected:
    ...
    - Default the type, severity, and points from the category if they are missing on the record.
